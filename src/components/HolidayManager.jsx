...
const HolidayManager = () => {
    ...
    const combineHolidays = (holidayList) => {
        ...
        return Object.values(holidayMap).map(holiday => {
            ...
            const dateString = dates.length > 2 ? `${start} - ${end}` : dates.join(', '); // Show range or list of dates
    ...
    const handleAddHolidays = async () => {
        ...
        try {
            ...
            await Promise.all(holidaysToAdd.map(holiday =>
                axios.post(`${API_URL}/add`, holiday)
            ));
        ...
        <div className="p-6 bg-gray-50 rounded-lg shadow-lg">
            ...
            <div className="mb-4 grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <input
                    ...
                    min="1"
                />
                <button onClick={handleAddHolidays} className="bg-blue-600 text-white p-3 rounded-lg shadow hover:bg-blue-700 transition duration-200">Add Holidays</button>
            </div>

            <h2 className="text-3xl font-semibold mt-6 mb-2">All Holidays</h2>
            <ul className="list-disc pl-5">
                {holidays.map((holiday) => (
                    <li key={holiday._id} className="flex justify-between items-center bg-white p-3 rounded-lg shadow mb-2">
